...
const pubClient = require('./helpers').pub;
const subClient = require('./helpers').sub;
...
exports.register = (server, options, next) => {
  ...
  io.of('/roomslist').on('connection', (socket) => {
    dbHelpers.getAllRooms((errR, rooms) => {
      socket.on('getChatRooms', (chatRooms, callback) => {
        if (chatRooms) {
          socket.join(chatRooms, callback);
  ...
  io.of('/chatter').on('connection', (socket) => {
    ...
    socket.on('join', data => {
      ...
      socket.on('newMessage', data => {
        data.message = dbHelpers.validateMessages(data.message);
